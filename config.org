...
Banner setting is configured in the Dashboard Customization section (line ~1048)
to ensure it loads after the theme but before the dashboard is displayed.
This prevents blank flashes on startup while still using the custom XPM logo.
...
(after! org-agenda
  ...
  ;; Force evil state in agenda
  (set-evil-initial-state! 'org-agenda-mode 'normal)

  ;; Some agenda views (e.g. `alltodo`, often used for overview dashboards)
  ;; don't reliably get Doom's `evil-org-agenda` navigation keys, so `j/k`
  ;; can fall back to Org's default bindings (like `org-agenda-goto-date`).
  ;; Bind `j/k` at the base agenda keymap level to keep navigation consistent.
           ...
           ((org-agenda-overriding-header "Active Tasks")
            ...
            (org-super-agenda-groups
             '((:name "Overdue" :deadline past :order 1)
               ...
               (:name "Unscheduled TODOs" :todo "TODO" :order 8)
...
(setq +doom-dashboard-menu-sections
      '(("Open project"
         :icon (nerd-icons-octicon "nf-oct-repo" :face 'doom-dashboard-menu-title :height 1.3)
         :action projectile-switch-project)
        ("Open documentation"
         :icon (nerd-icons-faicon "nf-fa-book" :face 'doom-dashboard-menu-title :height 1.3)
         :action doom/help)))
