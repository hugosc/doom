...
(defun my/org-add-blank-lines-around-src-blocks ()
  ...
  (save-excursion
    ...
    (goto-char (point-min))
    (while (re-search-forward "^[ \t]*#\\+END_SRC" nil t)
      (my/org--ensure-blank-line-after (match-end 0)))))
...
(add-hook 'org-mode-hook
          (lambda ()
            ...
            ;; Make sure our helpers run only for this buffer on save
            (add-hook 'before-save-hook #'my/org-add-blank-lines-around-src-blocks nil t)
            (add-hook 'before-save-hook #'my/org-ensure-blank-lines-between-paragraphs nil t)))
...
(after! org
  ;; Set up org-capture templates for inbox
   (setq org-capture-templates
         '(("i" "Inbox TODO" entry
           ...
           ("w" "Weekly TODO" entry
            ...
            "** TODO %?\n"
            :empty-lines 1)
            ("D" "Document (LaTeX export)" plain
             (file+head "concepts/%<%Y-%m-%d_%H%M%S>.org"
                        "#+TITLE: %^{Title}\n#+AUTHOR: %^{Author|Crocodile}\n#+DATE: %^{Date|%<%Y-%m-%d>}\n#+FILETAGS: :document:\n#+LATEX_CLASS: %^{LaTeX class|essay|article|report}\n#+OPTIONS: toc:%^{TOC|nil|t} num:%^{num|nil|t}\n")
             "%?\n"
              :unnarrowed t)))
   ...
   (defun my/org-format-buffer ()
    ...
    (interactive)
    (save-excursion
      ;; First, collapse runs of 3+ blank lines into exactly two.
   ...
   ;; NOTE: automatic `my/org-format-buffer` on-save removed to avoid
   ;; conflicting edits with paragraph/code-block spacing helpers.
   ;; If you prefer to run it manually, call M-x my/org-format-buffer.
   )
#+end_src
