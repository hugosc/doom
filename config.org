...
Each heading gets its own attachment directory."
  (interactive)
  ;; Ensure we're in an org file and under a heading
...
(defun my/paste-screenshot-naturally ()
  "Paste screenshot using org-download at current heading.
Creates heading ID and :ATTACH: tag if needed. Each heading gets its own directory."
  (save-excursion
    ;; Ensure we're at the heading
    (org-back-to-heading t)
    ;; Store heading info for message
    (let ((heading-title (org-get-heading t t t t)))
      ;; Move to end of heading to paste
      (org-end-of-subtree t t)
      ;; Call org-download-clipboard with timeout protection
      ...
      ;; Force refresh ALL images after paste
        ...
        (lambda ()
          (when (buffer-live-p (current-buffer))
            (with-current-buffer (current-buffer)
              (org-remove-inline-images)
              (org-display-inline-images t t (point-min) (point-max))))))
      (message "Screenshot pasted (to heading: %s)" heading-title))))
#+end_src
